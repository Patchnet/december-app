...
export class PocketSync {
  ...
  async pair() {
    ...
    let presentation
    try {
      presentation = await this.#createPresentation(created.spaceId, 1, claim, contentKey)
    } catch (error) {
  ...
  async #createPresentation(spaceId, epoch, claim, contentKey) {
    ...
    const capsule = createPairingCapsule({ pairingBundle: pairingUrl, now, expiresAt })
    // This upload is deliberately unauthenticated and contains only a public
    // routing selector plus opaque ciphertext. The manual secret, claim
    // secret, and root content key never enter the body.
    await responseJson(await this.#request(PAIRING_CAPSULE_PATH, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify(capsule.relay),
    }, false))
    return { pairingUrl, pairingCode: capsule.manualCode, pairingExpiresAt: capsule.expiresAt }
  }
