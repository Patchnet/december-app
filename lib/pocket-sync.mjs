...
function pocketMessage(value, item) {
  if (!value || value.v !== PROTOCOL_VERSION || typeof value.type !== 'string') throw new Error('invalid Pocket message')
  if (value.type === 'capture') {
    if (typeof value.text !== 'string') throw new Error('invalid Pocket capture')
    return {
  ...
  if (value.type !== 'action'
    || value.id !== item.captureId
    || value.action !== 'check'
    || typeof value.blockId !== 'string'
    || !value.blockId || value.blockId.length > 128
    || (value.itemId != null && (typeof value.itemId !== 'string' || !value.itemId || value.itemId.length > 128))
    || value.done !== true
    || (value.expectedWhen != null && !/^\d{4}-\d{2}-\d{2}$/.test(value.expectedWhen))) {
    throw new Error('invalid Pocket action')
  }
  return {
    id: value.id,
    type: 'action',
    ...
    expectedWhen: value.expectedWhen ?? null,
    at: value.createdAt || item.receivedAt,
  }
}
...
export class PocketSync {
  ...
  async init() {
    ...
    // `pendingMove` was written by an unreleased staged-move design that no
    // Relay implemented. Drop it during load so it cannot strand pairing.
  ...
  status() {
    return {
      ...
      phoneReady: !!this.state.phoneReadyAt,
      phoneReadyAt: this.state.phoneReadyAt,
  ...
  async checkPhoneReady() {
    if (!this.#paired()) return this.status()
    try {
      const response = await responseJson(await this.#request('/devices', { headers: this.#authHeaders() }))
      const epoch = this.state.space.epoch
      const phone = Array.isArray(response.items)
        ? response.items.find((device) => device?.role === 'pocket'
          && !device.revokedAt
          && device.readyEpoch === epoch
          && typeof device.readyAt === 'string')
        : null
      const readyAt = phone?.readyAt ?? null
      if (this.state.phoneReadyAt !== readyAt) {
        await this.#update((state) => { state.phoneReadyAt = readyAt })
      }
    } catch { /* Keep the last confirmed state until Relay answers authoritatively. */ }
    return this.status()
  }
  ...
  // Every replacement is one atomic Relay rotation. The old credential is
  // revoked before the replacement claim is returned.
  async rotate({ reason = 'reconnect' } = {}) {
    if (reason === 'lost-phone') return this.lostPhone()
    if (reason === 'move-device') return this.#rotateNow('move-device')
  ...
  async #pullCaptures(consume) {
    ...
    try {
      ...
      for (const item of batch.items || []) {
        ...
        await consume(pocketMessage(value, item))
  ...
  async #adopt({ spaceId, epoch, deviceId, desktopToken, contentKey }) {
    ...
    await this.#update((state) => {
      ...
      // Relay keeps page revisions monotonic across key rotation so an old
      // ciphertext cannot be replayed. Preserve the next local revision when
      // adopting a rotated key; fresh pairings already begin at one.
      state.nextPageRevision = Math.max(1, Number(state.nextPageRevision) || 1)
      state.pendingPage = null
      state.captureCursor = 0
      state.lastSyncedAt = null
      state.phoneReadyAt = null
      state.lastError = null
    })
  }
