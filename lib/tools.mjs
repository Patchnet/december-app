...
import { BLOCK_TYPES, createBlockFields, typeDocs, verbTools, verbPatch } from './blocks.mjs'
...
export const TOOLS = [
  ...
  },
  {
    ...
    description:
      'Close out a space (or reopen a closed one). A finished space keeps everything it holds and moves to the finished list at the bottom of the page. Only close one out when the person says it is done; never as tidying.',
    inputSchema: {
      type: 'object',
...
export async function callTool(name, args = {}, settleStatus = {}) {
  switch (name) {
    ...
    case 'december_set_reminder': {
      ...
      return core.updateBlock(v.blockId, v.patch, v.type)
    }
    ...
    case 'december_retitle':
      return core.updateBlock(args.blockId, { retitle: args.title })
