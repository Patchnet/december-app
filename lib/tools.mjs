import {RECEIPT_TOOLS} from './operation-receipts.mjs'
...
export const TOOLS = [
  ...
  },
  {
    name: 'december_focus',
    description:
      'Record ONE completed focus span: how long the person actually sat with one list item or one reminder, written once, after it ended. The page writes this itself when its spotlight closes — it is a receipt, not a timer. There is nothing to start and nothing to stop here. Never invent a span, never estimate one, and never log one the person did not tell you about.',
    inputSchema: {
      type: 'object',
      properties: {
        blockId: { type: 'string', description: 'the list or the reminder that holds the task' },
        itemId: { type: 'string', description: 'which list item; omit for a reminder' },
        ms: { type: 'number', description: 'how long the span lasted, in milliseconds' },
      },
      required: ['blockId', 'ms'],
      additionalProperties: false,
    },
  ...
  },
  {
    ...
    inputSchema: {
      ...
      // Only the question is required. The description tells an assistant to
      // omit options for a typed answer, and setAsk accepts their absence; a
      // strict client must not be forced to send an empty array instead.
      required: ['question'],
      additionalProperties: false,
    },
  },
  ...
  {
    name: 'december_undo',
    description: 'Undo the most recent agent changes while preserving independent later work. Refuses conflicts with newer edits. One level only.',
    inputSchema: { type: 'object', properties: {}, additionalProperties: false },
  },
]

/** Dispatch a tool call against the core. settleStatus feeds december_view. */
for(const tool of TOOLS) if(RECEIPT_TOOLS.has(tool.name)) {
  tool.inputSchema.properties.operationId={type:'string',description:'Stable ID for one intended action; reuse on retry, use distinct IDs for separate identical actions. Supply for retry protection when there is no capture source.'}
  if(!tool.inputSchema.properties.source && tool.name!=='december_file_capture')tool.inputSchema.properties.source={type:'string',description:'Original capture ID, when acting on a capture.'}
}
export async function callTool(name,args={},settleStatus={}) {
  return core.executeAgentOperation(name,args,()=>dispatchTool(name,args,settleStatus))
}
async function dispatchTool(name, args = {}, settleStatus = {}) {
  switch (name) {
    case 'december_view':
    ...
    case 'december_set_reminder': {
      if (name === 'december_set_reminder' && args.itemId !== undefined) return core.scheduleListItem(args)
