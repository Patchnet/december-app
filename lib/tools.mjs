  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        reminder_when: { type: 'string', description: 'due date YYYY-MM-DD' },
        reminder_repeat: { type: 'string', enum: ['daily', 'weekly', 'monthly', 'yearly', ''], description: 'recurrence; empty clears' },
        retitle: { type: 'string' },
        source: { type: 'string', description: 'The captureId this change comes from, so it stays traceable.' },
      },
      required: ['blockId'],
      additionalProperties: false,
    },
