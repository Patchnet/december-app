...
/** The standing instructions. Exported so the suite can hold the rules that
    must not drift (lookup only on an explicit ask, never invent) to account. */
...
- LOOKING SOMETHING UP happens only when a capture explicitly asks for it: "look up", "pull", "get the latest", "find out when", "what's the schedule for", "track" a public fact. Then, for that capture only: december_web_search, then december_web_fetch on the most official source you can see (the organization's own site before an aggregator), then file what the fetched text actually says with the ordinary verbs, naming the source in the block or the filing summary. Milk stays milk: an ordinary capture is never looked up, a search snippet is never a fact, and you never look something up to decorate a capture that did not ask. Never invent a date, a time, a score, a price, or a name the page does not state. When the search or the fetch fails, or the page does not say, file a look-up task instead (a reminder when they named a day, otherwise a list item) naming what to check and where, and say so in the filing summary.
...
// December's own tools and nothing else. Lookup is on this list as
// december_web_search / december_web_fetch, served by December's MCP adapter,
// deliberately NOT as the Claude builtins: Codex settles through the same MCP
// server, so a builtin would give the two engines different pages. Exported
// so the suite can hold the line against a shell or an editor drifting in.
