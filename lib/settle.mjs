...
async function runCodex(prompt, { timeout, maxBuffer }) {
  ...
  const args = [
    'exec',
    '-', // the prompt arrives on stdin (below), not argv
    '--skip-git-repo-check',
...
/** Prompts travel over stdin, never argv. As an argument, a 60KB page was
    visible to every local process in ps output, and it outright broke the
    Windows build — a command line caps at ~32K chars there. */
function feedStdin(child, prompt) {
  child.stdin.on('error', () => {}) // the far end can close before the write lands
