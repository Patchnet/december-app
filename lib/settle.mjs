...
- Goals. The view lists goals first: each is a target over one block, with where it stands and how long since it moved. When a capture reports progress in a goal's unit or space, it goes into THAT goal's block with the block's own verb; never create a second block for what a goal already counts. When the person names a total they want to reach ("my goal this year is 200 miles", "12 books by December", "$10k saved by October"), december_set_goal on the block that counts it, creating that block first if the space has none. A goal that has not moved in a long while is worth a gentle december_surface line, not a question.
...
const ALLOWED = [
  ...
  'retitle', 'rename', 'suggest', 'ask', 'set_area', 'pin', 'finish', 'surface', 'write_about', 'set_goal',
]
