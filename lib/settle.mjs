...
- Extract numbers. Yearly goals get yearly targets. Reminders that imply a date get when (YYYY-MM-DD); ones that name a clock time ("9am", "at 3:30", "dentist tuesday morning" is NOT a time) also get at (HH:MM, 24h); ones that recur ("every month") also get repeat (daily|weekly|monthly|yearly). A capture's hint names the space the person was viewing: strongly prefer it.
...
const ALLOWED = [
  ...
  'retitle', 'rename', 'suggest', 'ask', 'set_area', 'pin', 'finish', 'surface',
]
...
What does NOT belong: reminders due today or tomorrow (the page already shows those itself), progress commentary, encouragement, and anything a card already states plainly. Prefer the concrete and actionable — "call the landlord back" over "review housing".
...
export async function runSurface() {
  ...
  try {
    ...
    if (a === -1 || b <= a) throw new Error('no list returned')
    const list = JSON.parse(raw.slice(a, b + 1))
