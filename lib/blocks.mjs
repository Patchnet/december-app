...
export const BLOCKS = {
  ...
  reminder: {
    doc: 'a line that resurfaces until done; give it when (YYYY-MM-DD) if it implies a date',
    fields: { when: { type: 'string', description: 'reminder: due date YYYY-MM-DD, when the capture implies one' } },
    make: (spec) => ({ text: String(spec.text || spec.title || '').slice(0, 200), done: false, when: spec.when ? String(spec.when).slice(0, 10) : '' }),
    update: (b, p) => {
