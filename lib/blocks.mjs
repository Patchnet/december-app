...
export const BLOCKS = {
  list: {
    ...
    make: (spec) => ({
      items: (spec.items || []).slice(0, 30).map((t) => ({ id: uid(), text: String(t).slice(0, 200), done: false })),
    }),
    update: (b, p) => {
      for (const t of p.add_items || []) b.items.push({ id: uid(), text: String(t).slice(0, 200), done: false })
  ...
  ledger: {
    ...
    update: (b, p) => {
      if (p.entry_label != null || p.entry_amount != null) {
        b.entries.push({ id: uid(), label: String(p.entry_label || '').slice(0, 80), amount: Number(p.entry_amount) || 0, at: new Date().toISOString() })
      }
    },
  ...
  reminder: {
    doc: 'a line that resurfaces until done',
    fields: {},
    make: (spec) => ({ text: String(spec.text || spec.title || '').slice(0, 200), done: false }),
    update: (b, p) => {
