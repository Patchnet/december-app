...
export function makeBlock(spec) {
  ...
  return { id: uid(), type: spec.type, title: String(spec.title || '').slice(0, 80), ...def.make(spec), entities }
}
...
/** The goal as the page and the engine read it: where it stands, where it
    should stand today, and whether that is ahead or behind. */
export function goalOf(block, now = new Date()) {
  if (!block?.goal) return null
  const g = block.goal
  const current = goalMeasure(block)
