...
// Existing short ids remain valid. New ids use the operating system's
// cryptographic random source because they cross-reference mutable records.
export const uid = () => randomBytes(16).toString('base64url')

const today = () => new Date().toISOString().slice(0, 10)
const ENTITY_TYPES = ['person', 'org', 'place', 'thing']

/** A stable month/day for calendar rhythms. The due date may clamp in a
    short month, but this anchor must not: Jan 31 -> Feb 28 -> Mar 31. */
export function recurrenceAnchorOf(value) {
...
// ------------------------------------------------------------------ watch
// A watch is a TAG on a block, never a seventh type. The block's own words
// are the query, and the tag is the only thing that lets December go and
// look again: an untagged block never reaches the network, and a tag stops
// mattering the moment it is cleared or its space is archived.

export const watchField = {
  type: 'boolean',
  description:
    'true ONLY when the capture asks to keep a public fact current ("track the UF home schedule", "keep an eye on the recall notice"). The block\'s own text is the query and December refreshes it on its own until this is set false or the space is archived. An ordinary capture is never watched: milk stays milk. Set false when the person says to stop.',
}

/** Lay a watch over a block, or lift it. The tag stays small on purpose:
    when it began, and nothing else. How a refresh went is bookkeeping for
    the watch pass, not the person's page. */
export function setWatch(block, on) {
  if (on) block.watch = block.watch || { since: new Date().toISOString() }
  else delete block.watch
  return block.watch || null
}

/** What a watched block asks the web. The first line of the person's own
    words, because that is the thing they said they wanted kept current. */
export function watchQuery(block) {
  const own = block?.type === 'note' || block?.type === 'reminder' ? block.text || block.title : block?.title || block?.text
  return String(own || '').split('\n')[0].trim().slice(0, 200)
}
...
export const BLOCKS = {
  list: {
    ...
    make: (spec, { preserveListItems = false } = {}) => ({
      // Carryover supplies open item records so their source remains visible.
      // Ordinary create calls still pass strings and keep the same shape.
      items: (preserveListItems ? (spec.items || []) : (spec.items || []).slice(0, 30)).map((value) => {
        const item = value && typeof value === 'object' && !Array.isArray(value) ? value : { text: value }
        return {
          id: uid(),
          text: String(item.text || '').slice(0, 200),
          done: false,
          src: String(item.src || spec.source || '').slice(0, 200),
        }
      }),
    }),
    verb: {
  ...
  reminder: {
    ...
    make: (spec) => {
      const when = spec.when ? String(spec.when).slice(0, 10) : ''
      const repeat = ['daily', 'weekly', 'monthly', 'yearly'].includes(spec.repeat) ? spec.repeat : ''
      const repeatAnchor = repeat ? recurrenceAnchorOf(when) : ''
      return {
    ...
    verb: {
      name: 'set_reminder',
      doc: 'update an existing reminder; to schedule an existing list task, pass its list blockId and itemId to move that exact task into a reminder, preserving its history instead of creating a duplicate',
      params: {
        itemId: { type: 'string', description: 'exact list item id to move into a reminder; blockId is its list, or an existing reminder in the same space to explicitly consolidate a duplicate; omit for an ordinary reminder edit' },
        text: { type: 'string', description: 'optional updated task wording; preserve the intent and details' },
        done: { type: 'boolean' },
      ...
      map: (a) => ({ reminder_done: a.done, reminder_when: a.when, reminder_at: a.at, reminder_repeat: a.repeat, ...(a.text !== undefined ? { reminder_text: a.text } : {}) }),
    },
    update: (b, p) => {
      if (p.reminder_text != null) b.text = String(p.reminder_text).slice(0, 200)
      if (p.reminder_done != null) {
        b.done = !!p.reminder_done
        b.doneAt = b.done ? new Date().toISOString() : null
      }
      const whenChanged = p.reminder_when != null
      const repeatChanged = p.reminder_repeat != null
      if (whenChanged) b.when = String(p.reminder_when).slice(0, 10)
      if (p.reminder_at != null) b.at = /^\d{2}:\d{2}$/.test(p.reminder_at) ? p.reminder_at : ''
      if (repeatChanged) b.repeat = ['daily', 'weekly', 'monthly', 'yearly', ''].includes(p.reminder_repeat) ? p.reminder_repeat : b.repeat
      if (!b.repeat) delete b.repeatAnchor
      else if (whenChanged || repeatChanged) {
        const anchor = recurrenceAnchorOf(b.when)
        if (anchor) b.repeatAnchor = anchor
        else delete b.repeatAnchor
      }
    },
  },
}
...
function buildBlock(spec, options) {
  const def = BLOCKS[spec?.type]
  if (!def) return null
  const entities = Object.hasOwn(spec, 'entities') ? validateEntities(spec.entities) : []
  const block = { id: uid(), type: spec.type, title: String(spec.title || '').slice(0, 80), ...def.make(spec, options), entities }
  ...
  // and a block born from "keep track of the schedule" is watched from it
  if (spec.watch === true) setWatch(block, true)
  return block
}

export function makeBlock(spec) {
  return buildBlock(spec)
}

/** Year Close is an internal restoration path, so it must not apply the
    user-facing initial-list cap to already selected unfinished items. */
export function makeCarryoverBlock(spec) {
  return buildBlock(spec, { preserveListItems: true })
}
...
export function updateBlock(block, patch) {
  ...
  if (patch.set_watch != null) setWatch(block, patch.set_watch === true)
}
...
export function createBlockFields() {
  ...
  fields.watch = watchField // any type can be watched; none is watched by default
  return fields
}
...
export function verbTools() {
  return Object.entries(BLOCKS)
    ...
    .map(([type, def]) => ({
      ...
      description: `${def.verb.doc}. ${type === 'reminder' ? 'Use a reminder blockId, or a list blockId with itemId to schedule that task.' : `Works on a ${type} block; get its id from december_view.`}`,
      inputSchema: {
        type: 'object',
        properties: {
          blockId: { type: 'string' },
          ...
          // the tag rides on every verb, so the block that already holds the
          // thing is also where it is said to keep it current, or to stop
          watch: watchField,
...
export function verbPatch(toolName, args) {
  for (const def of Object.values(BLOCKS)) {
    if (def.verb && `december_${def.verb.name}` === toolName) {
      const { blockId, source, entities, watch, ...rest } = args
      ...
      return {
        ...
        patch: {
          ...
          ...(Object.hasOwn(args, 'watch') ? { set_watch: watch === true } : {}),
        },
      }
    }
  }
...
export function goalOf(block, now = new Date()) {
  ...
  // These are projection-only readings. Keep them out of block.goal so old
  // state files retain their exact durable shape while every consumer reads
  // the same pace boundary and wording.
  const whole = Number.isInteger(current) && Number.isInteger(target)
  const gap = whole ? Math.round(Math.abs(diff)) : Math.abs(diff)
  const met = current >= target
  const paceWord = met ? 'met' : gap < 0.5 ? 'on' : diff > 0 ? 'ahead' : 'behind'
  const behind = !met && diff < -0.5
  const since = g.movedAt || (g.setAt && `${g.setAt}T12:00:00`)
  const quietDays = since ? Math.max(0, Math.floor((now - new Date(since)) / 86400000)) : null
  const pace = paceWord === 'met' ? 'met' : paceWord === 'on' ? 'on pace' : `${gap} ${paceWord}`
  const shownGap = gap >= 1000
    ? gap.toLocaleString('en-US', { maximumFractionDigits: 0 })
    : String(Math.round(gap * 100) / 100)
  const paceText = paceWord === 'met'
    ? 'done'
    : paceWord === 'on'
      ? 'on pace'
      : `${g.unit === '$' ? '$' : ''}${shownGap} ${paceWord}`
  const quietText = !met && quietDays != null && quietDays >= 14 ? `quiet ${quietDays} days` : ''
  return {
    current, target, unit: g.unit, from: g.from, by: g.by, base: g.base,
    through, expected: Math.round(expected * 10) / 10, diff,
    whole, gap, paceWord, pace, paceText, behind, quietDays, quietText,
    met, movedAt: g.movedAt, setAt: g.setAt,
  }
}
