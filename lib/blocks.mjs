...
export function goalOf(block, now = new Date()) {
  ...
  // The readings every voice speaks from — the band, the year card, the
  // engine's view — derived ONCE here so they can never disagree:
  // gap rounds whole when the goal counts in wholes; paceWord is the verdict
  // ('met'|'on'|'ahead'|'behind'); quietDays counts from the last move, or
  // from the day the goal was set if it has never moved.
  ...
  return {
    ...
    through, expected: Math.round(expected * 10) / 10, diff, gap, paceWord, quietDays,
