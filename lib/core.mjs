...
import {operationIdentity} from './operation-receipts.mjs'
import {undoChanges,reverseChanges} from './undo-changes.mjs'
...
import {scheduleSignature,scheduledItem} from './task-edits.mjs'
import { validateCaptureText, CAPTURE_LINE_LIMIT } from '../public/js/capture-input.js'
import { readFileSync, existsSync, mkdirSync, readdirSync, createReadStream, renameSync } from 'node:fs'
import { appendFile, link, unlink, writeFile, rename } from 'node:fs/promises'
import { dirname, join, resolve } from 'node:path'
import { fileURLToPath } from 'node:url'
import { createInterface } from 'node:readline'
import { isDeepStrictEqual } from 'node:util'
import { uid, makeBlock, makeCarryoverBlock, updateBlock as applyBlockPatch, projectBlock, goalMeasure, goalOf, setBlockGoal, GOAL_TYPES, recurrenceAnchorOf } from './blocks.mjs'
...
const emptyState = () => ({
  ...
  answers: [], // recent question response receipts
  operations: [], // durable agent operation receipts; retained across rollover
  pocketActions: [], // bounded encrypted-phone action receipts: {id, at, status}
  ...
  activity: [], // {at, space, summary} + captureId when a capture was filed, kind when it was not
...
state.pocketActions = Array.isArray(state.pocketActions) ? state.pocketActions.slice(-256) : []
state.answers = Array.isArray(state.answers) ? state.answers : []
const durableAnswers = new WeakSet(state.answers)
state.operations = Array.isArray(state.operations) ? state.operations : []
const durableOperations = new Set(state.operations.map(receipt=>receipt.id))
const operationContext = new AsyncLocalStorage()
const operationWrites = new Map()
let runIdentity = null
export function beginAgentRun() { runIdentity=uid(); return runIdentity }

/** Receipts and mutations share the same atomic state-file replacement. */
export async function executeAgentOperation(name,args,action) {
  const identity=operationIdentity(name,args)
  if(!identity)return operationContext.run({agent:true},action)
  const pending=operationWrites.get(identity.id)
  if(pending) {
    if(pending.signature!==identity.signature)throw new Error('operationId already belongs to different arguments')
    return structuredClone(await pending.write)
  }
  const prior=state.operations.find(receipt=>receipt.id===identity.id)
  if(prior) {
    if(prior.signature!==identity.signature)throw new Error('operationId already belongs to different arguments')
    if(prior.status==='undone')throw new Error('This operation was undone. Read the latest page; use a new operationId only for an intentional new action.')
    if(!durableOperations.has(prior.id))await persist()
    return structuredClone(prior.result)
  }
  const capture=identity.source && state.captures.find(c=>c.id===identity.source)
  if(capture?.status==='filed')throw new Error('This capture is already filed. Read its saved result before doing more work.')
  if(state.operations.length>=10000 || identity.source && state.operations.filter(r=>r.source===identity.source).length>=64)throw new Error('Operation receipt limit reached; no new change was applied.')
  if(JSON.stringify(identity).length>32000)throw new Error('Operation is too large; no change was applied.')
  const context={agent:true,identity}
  const write=operationContext.run(context,async()=>{
    const result=await action()
    if(!context.recorded)throw new Error('Operation did not produce a durable receipt')
    return result
  })
  operationWrites.set(identity.id,{signature:identity.signature,write})
  try{return structuredClone(await write)}finally{operationWrites.delete(identity.id)}
}

async function agentResult(event,result) {
  const context=operationContext.getStore()
  if(context?.identity && !context.recorded) {
    state.operations.push({...context.identity,result:structuredClone(result),status:'applied',at:new Date().toISOString()})
...
// Receipts loaded from state.json are known durable. A newly-created receipt
// joins this set only after its state write succeeds.
const durableCaptures = new WeakSet(state.captures)
const captureWrites = new WeakMap()
const durablePocketActionIds = new Set(state.pocketActions.map((receipt) => receipt.id))
...
const manualStack = []
const undoView = () => structuredClone({spaces:state.spaces,captures:state.captures,retired:state.retired,lessons:state.lessons,about:state.about})
function manualEntry(label) { return {label,before:undoView(),year:state.yearOf} }
function keepManual(entry) {
  if(operationContext.getStore()?.agent)return keepAgent(entry)
  entry.changes=undoChanges(entry.before,undoView())
  delete entry.before
  if (!entry.changes.length) return
  manualStack.push(entry)
  if (manualStack.length>15) manualStack.shift()
}
function applyUndo(entry,operations) {
  if (entry.year!==state.yearOf) throw new Error('This undo belongs to a previous year.')
  const next=reverseChanges(undoView(),operations)
  const now=new Date().toISOString()
  for (const space of next.spaces) {
    const old=state.spaces.find(item=>item.id===space.id)
    if (!isDeepStrictEqual(space,old)) space.updatedAt=now
  }
  Object.assign(state,next)
  for(const id of entry.receiptIds || []) {
    const receipt=state.operations.find(r=>r.id===id)
    if(receipt)receipt.status='undone'
  }
}
export async function undoManual() {
  const top=manualStack.at(-1)
  if (!top) throw new Error('nothing of yours to undo')
  if (top.writing) return top.writing
  if (!top.applied) { applyUndo(top,[top.changes]); top.applied=true }
  top.writing=persistEvent({kind:'undo_manual',summary:top.label}).then(()=>{
    const index=manualStack.indexOf(top)
    if(index>=0)manualStack.splice(index,1)
    return {label:top.label}
  }).finally(()=>{top.writing=null})
  return top.writing
}
...
function persist({ rolloverMutation = true } = {}) {
  ...
  const includedCaptures = [...state.captures]
  const includedOperations = state.operations.map(receipt=>receipt.id)
  const includedAnswers = [...state.answers]
  const done = writing.then(async () => {
    ...
    for (const capture of includedCaptures) durableCaptures.add(capture)
    for (const id of includedOperations) durableOperations.add(id)
    for (const receipt of includedAnswers) durableAnswers.add(receipt)
...
export function project(settleStatus = {}) {
  return {
    ...
    // Encrypted with the rest of the Pocket page. The phone uses these
    // bounded receipts to replace an optimistic check with a durable result.
    pocketActions: state.pocketActions.slice(-64),
    pocketCapabilities: { checkActions: 1 },
...
export function agentView() {
  ...
  return {
    captures: state.captures.filter((c) => c.status === 'inbox').map(c=>({...c,appliedOperations:state.operations.filter(r=>r.source===c.id && r.status==='applied').map(({operationId,tool,arguments:args,result})=>({operationId,tool,arguments:args,result}))})),
...
/** Record each mutation separately. Settle passes have explicit boundaries;
    connected assistants retain the short inactivity boundary. Manual changes
    between calls never become part of an agent inverse. */
function agentBatchGuard() { return manualEntry('agent') }
function keepAgent(entry) {
  const changes=undoChanges(entry.before,undoView())
  if (!changes.length) return
  const now=Date.now()
  if (!previousSnap || previousSnap.applied || previousSnap.year!==state.yearOf || previousSnap.runId!==runIdentity || now-lastAgentWriteAt>60000) {
    previousSnap={operations:[],receiptIds:[],runId:runIdentity,at:now,year:state.yearOf}
  }
  previousSnap.operations.push(changes)
  const receipt=operationContext.getStore()?.identity
  if(receipt)previousSnap.receiptIds.push(receipt.id)
...
export async function undo() {
  ...
  const entry=previousSnap
  if(entry.writing)return entry.writing
  if(!entry.applied){applyUndo(entry,entry.operations);entry.applied=true}
  entry.writing=persistEvent({kind:'undo'}).then(()=>{
    if(previousSnap===entry)previousSnap=null
...
/** Prepare intake without losing existing captures. Batches use a staging array. */
function intakeCapture(value, hint, source = {}, captures = state.captures) {
  const text = validateCaptureText(value)
  const suppliedId = String(source.id || '').trim()
  if (suppliedId.length > 256) throw Object.assign(new Error('capture id is too long'), {statusCode:400})
  const normalizedHint = hint ? String(hint).slice(0,60) : undefined
  if (suppliedId) {
    const existing = captures.find(capture => capture.id === suppliedId)
    if (existing) {
      if (existing.text !== text || (existing.hint || '') !== (normalizedHint || '')) throw Object.assign(new Error('capture id already belongs to different content'), {statusCode:409})
      return {capture:existing,duplicate:true}
    }
  }
  const capture = {id:suppliedId || uid(),text,at:source.at ? new Date(source.at).toISOString() : new Date().toISOString(),status:'inbox'}
  // Old clients without IDs retain their brief double-click protection.
  // Distinct explicit IDs represent distinct authored notes, even if identical.
  if (!suppliedId) {
    const echo = captures.find(c => {
      const elapsed = new Date(capture.at) - new Date(c.at)
      return c.status === 'inbox' && c.text === text && (c.hint || '') === (normalizedHint || '') && elapsed >= 0 && elapsed < 5000
    })
    if (echo) return {capture:echo,duplicate:true}
  }
  if (captures.filter(c => c.status === 'inbox').length >= 200) throw Object.assign(new Error('The inbox has 200 unfiled notes. File some before retrying; nothing was removed.'), {statusCode:409})
  if (normalizedHint) capture.hint = normalizedHint
  if (source.requestId) {capture.requestId=source.requestId;capture.requestDigest=source.requestDigest}
  captures.push(capture)
  return {capture,duplicate:false}
}

async function retainCapture(capture) {
  if (captureWrites.has(capture)) return captureWrites.get(capture)
  if (durableCaptures.has(capture)) return
  const write = persistEvent({kind:'capture',itemId:capture.id,summary:capture.text.slice(0,200)})
  captureWrites.set(capture,write)
  try { await write } finally { if (captureWrites.get(capture) === write) captureWrites.delete(capture) }
}

export async function addCapture(text, hint, source = {}) {
  const {capture,duplicate} = intakeCapture(text,hint,source)
  await retainCapture(capture)
  return duplicate ? {...capture,duplicate:true} : capture
}

/** Validate the complete batch before changing the inbox. A stable request ID
    makes a lost response safe to retry, including after filing or restart. */
export async function addCaptureBatch(texts, hint, source = {}) {
  if (!Array.isArray(texts) || !texts.length || texts.length > CAPTURE_LINE_LIMIT) throw Object.assign(new Error('File between 1 and 25 lines at a time; nothing was accepted.'), {statusCode:413})
  const lines = texts.map(validateCaptureText)
  const requestId = String(source.id || '').trim()
  if (requestId.length > 128) throw Object.assign(new Error('request id is too long'), {statusCode:400})
  const requestDigest = createHash('sha256').update(JSON.stringify([lines,hint ? String(hint).slice(0,60) : ''])).digest('hex')
  const prior = requestId && state.captures.find(c => c.requestId === requestId)
  if (prior && prior.requestDigest !== requestDigest) throw Object.assign(new Error('request id already belongs to different content'), {statusCode:409})
  const staged = [...state.captures], kept = [], all = []
  for (const [index,text] of lines.entries()) {
    const result = intakeCapture(text,hint,requestId ? {id:`${requestId}:${index}`,requestId,requestDigest} : {},staged)
    all.push(result.capture)
    if (!result.duplicate) kept.push(result.capture)
  }
  // Another call may still own the first durable write for a duplicate.
  if (!kept.length) {
    for (const capture of all) await retainCapture(capture)
    return requestId ? all.map(capture => ({...capture,duplicate:true})) : []
  }
  state.captures = staged
  const write = persist()
  for (const capture of all) if (!durableCaptures.has(capture)) captureWrites.set(capture,write)
  try {
    await write
    for (const capture of kept) await appendEvent({kind:'capture',itemId:capture.id,summary:capture.text.slice(0,200)})
  } finally {
    for (const capture of all) if (captureWrites.get(capture) === write) captureWrites.delete(capture)
  }
  return requestId ? all : kept
}

/** The year, month by month: how much happened and a few words of what. */
function yearSummary() {
  const y = new Date().getFullYear()
  const months = Array.from({ length: 12 }, () => ({ events: 0, highlights: [], overdue: 0 }))
  ...
  for (const s of state.spaces) {
    if (s.finished) continue
    for (const b of s.blocks) {
      if (b.type === 'reminder' && !b.done && b.when) {
        if (b.when < localDay()) {
          const due = new Date(`${b.when}T12:00:00`)
          if (due.getFullYear() === y) months[due.getMonth()].overdue++
        } else aheadOf(b.when)
      }
...
export async function createSpace(name) {
  const agentEntry=agentBatchGuard()
  const space = resolveSpace(name)
  keepAgent(agentEntry)
  return agentResult({ kind: 'create_space', spaceId: space.id, summary: space.name }, { id: space.id, name: space.name })
}
...
export async function createBlock(spaceRef, spec) {
  ...
  }, { spaceId: space.id, blockId: block.id })
}
...
export async function updateBlock(blockId, patch, expectType, kind = 'update_block') {
  ...
  keepAgent(agentEntry)
  const result=await agentResult({ kind, ...blockEvent(hit, { src: patch.source || undefined }) }, {ok:true,blockId,finished:!!hit.space.finished})
  if (archived) await appendEvent({ kind:'finish',spaceId:hit.space.id,summary:'finished' })
  return result
}

/** Turn the exact existing task into a dated reminder in one state write. */
export async function scheduleListItem(args) {
  const {blockId,itemId}=args
  if (args.done !== undefined && args.done !== false) throw new Error('Scheduling a task does not complete it.')
  // An interrupted response may be retried after the original list went away.
  const prior=state.spaces.flatMap(space=>space.blocks.map(block=>({space,block})))
    .find(({block})=>block.movedFrom?.requestTarget===blockId && block.movedFrom.item.id===itemId)
  if (prior) {
    if (prior.block.movedFrom.signature!==scheduleSignature(args)) throw new Error(`This task already moved. Update reminder ${prior.block.id} instead.`)
    return agentResult({kind:'schedule_task_replay',blockId:prior.block.id},{ok:true,spaceId:prior.space.id,blockId:prior.block.id,moved:true})
  }
  const target=findBlock(blockId)
  const existing=target?.block.type==='reminder' ? target.block : null
  if (existing?.done || existing?.movedFrom) throw new Error('Read the latest card before consolidating this task.')
  // Supplying an existing reminder and the exact duplicate item is an explicit
  // consolidation. Never infer a match from similar task text.
  const list=existing ? target.space.blocks.find(block=>block.type==='list' && block.items.some(item=>item.id===itemId)) : target?.block
  const hit=list ? {space:target.space,block:list} : null
  if (!hit || hit.block.type!=='list') throw new Error('Choose the list block that holds this task, or its existing reminder in the same space.')
  const item=hit.block.items.find(item=>item.id===itemId)
  if (!item) throw new Error('This task is no longer in that list. Read the latest card before editing.')
  if (hit.space.finished) throw new Error('Reopen this space before scheduling a task.')
  if (hit.block.goal || trackerCounting(hit.space,hit.block) || hit.block.watch) throw new Error('This list is counted by a goal or tracker, or watched. Keep its task in place and clarify how to change it.')
  const reminder=scheduledItem(hit.block,item,existing ? {...existing,...args,text:args.text ?? existing.text} : args)
  reminder.movedFrom.requestTarget=blockId
  reminder.movedFrom.signature=scheduleSignature(args)
  if (existing) {
    reminder.id=existing.id
    reminder.movedFrom.previousReminder=structuredClone(existing)
  }
  const agentEntry=agentBatchGuard()
  hit.block.items=hit.block.items.filter(entry=>entry.id!==itemId)
  if (existing) {
    hit.space.blocks[hit.space.blocks.indexOf(existing)]={...existing,...reminder}
    if (!hit.block.items.length) hit.space.blocks.splice(hit.space.blocks.indexOf(hit.block),1)
  } else if (!hit.block.items.length) hit.space.blocks.splice(hit.space.blocks.indexOf(hit.block),1,reminder)
  else hit.space.blocks.push(reminder)
  touch(hit.space)
  keepAgent(agentEntry)
  return agentResult({kind:'schedule_task',spaceId:hit.space.id,blockId:reminder.id,itemId,src:args.source || item.src || undefined,summary:reminder.text},{ok:true,spaceId:hit.space.id,blockId:reminder.id,moved:true})
}
...
export async function fileCapture(captureId, spaceRef, summary) {
  ...
  if(c.status==='filed'){
    const prior=state.spaces.find(s=>s.id===c.spaceId)
    return agentResult({kind:'file_capture_replay',itemId:captureId},{ok:true,space:prior?.name || String(spaceRef)})
  }
  const space = resolveSpace(spaceRef)
  c.status = 'filed'
  c.spaceId = space.id
  ...
  keepAgent(agentEntry)
  return agentResult({ kind:'file_capture',spaceId:space.id,itemId:captureId,src:captureId,summary:c.summary },{ok:true,space:space.name})
}
...
export async function addLesson(text) {
  ...
  keepAgent(agentEntry)
  return agentResult({kind:'learn',summary:String(text).slice(0,200)},{ok:true})
}
...
export async function writeAbout(text, mode = 'set', { manual = false } = {}) {
  ...
  if (mode === 'append' && !incoming.trim()) {
    if(operationContext.getStore()?.identity)return agentResult({kind:'write_about',summary:'unchanged'},projectAbout())
    return projectAbout()
  }
  const undoEntry=manualEntry('about')
  let next
  if (mode === 'append') {
  ...
  if(manual)keepManual(undoEntry)
  else keepAgent(undoEntry)
  return agentResult({kind:'write_about',summary:mode==='append'?'appended':'set'},projectAbout())
}
...
/** Consume exactly the displayed question and retain its answer in one write.
    Validation happens first; an old response never dismisses a newer ask. */
export async function answerAsk({askId,choice='',typed=false}={}) {
  if(typeof askId!=='string' || !askId || askId.length>200)throw Object.assign(new Error('Reload the question before answering.'),{statusCode:409})
  if(typeof choice!=='string' || typeof typed!=='boolean')throw Object.assign(new Error('Invalid answer.'),{statusCode:400})
  const signature=createHash('sha256').update(JSON.stringify([choice,typed])).digest('hex')
  const prior=state.answers.find(receipt=>receipt.id===askId)
  if(prior) {
    if(prior.signature!==signature)throw Object.assign(new Error('This question already has a different response.'),{statusCode:409})
    if(!durableAnswers.has(prior))await persist()
    return {...prior,duplicate:true}
  }
  const asked=state.ask
  if(!asked || asked.id!==askId)throw Object.assign(new Error('That question has changed. Your answer was not applied to the new question.'),{statusCode:409})
  const staged=[...state.captures]
  const text=choice.trim() ? (typed ? `${asked.question} ${choice.trim()}` : choice.trim()) : ''
  const capture=text ? intakeCapture(text,undefined,{id:`answer:${askId}`},staged).capture : null
  const receipt={id:askId,signature,captureId:capture?.id || null,at:new Date().toISOString()}
  state.captures=staged
  state.ask=null
  state.answers=[...state.answers,receipt].slice(-256)
  await persistEvent({kind:'answer',itemId:receipt.captureId || undefined,summary:asked.question})
  return receipt
}
...
export async function setPinned(id, pinned) {
  ...
  const undoEntry=manualEntry('pin')
  sp.pinned = !!pinned
  keepManual(undoEntry)
...
export async function setFinished(id, finished) {
  const sp = state.spaces.find((s) => s.id === id)
  if (!sp) throw new Error('unknown space')
  const undoEntry=manualEntry('finish')
...
function applyCheckMutation(hit, itemId, done, { expectedWhen } = {}) {
  const measures = goalMeasures(hit.space)
  let changed = false
  if (hit.block.type === 'list') {
    ...
    if (it.done === next) return { changed: false, archived: false }
    it.done = next
    ...
    const b = hit.block
    if (expectedWhen && b.when !== expectedWhen) return { changed: false, archived: false, stale: true }
    if (done !== false && b.repeat && b.when) {
      ...
      const next = done !== false
      if (b.done === next) return { changed: false, archived: false }
      b.done = next
  ...
  if (!changed) return { changed: false, archived: false }
  stampGoalMoves(hit.space, measures)
  touch(hit.space)
  const archived = maybeArchiveComplete(hit.space)
...
export async function check(blockId, itemId, done) {
  ...
  const undoEntry = manualEntry('check')
  const result = applyCheckMutation(hit, itemId, done)
  if (!result.changed) return
  keepManual(undoEntry)
  await persistEvent({ kind: 'check', ...blockEvent(hit, { itemId: itemId || undefined, summary: done === false ? 'unchecked' : 'checked' }) })
  if (result.archived) await persistEvent({ kind: 'finish', spaceId: hit.space.id, summary: 'finished' })
}

/** Apply one encrypted Pocket action exactly once. Unknown or stale targets
    become durable receipts instead of wedging every later phone message. */
export async function applyPocketAction(id, action) {
  if (typeof id !== 'string' || !id || id.length > 256) throw new Error('invalid Pocket action id')
  const prior = state.pocketActions.find((receipt) => receipt.id === id)
  if (prior) {
    // A failed first write leaves the mutation in memory. Do not let the
    // Relay cursor acknowledge that volatile receipt until a retry proves
    // the current page and receipt are on disk.
    if (!durablePocketActionIds.has(id)) {
      await persist()
      durablePocketActionIds.add(id)
    }
    return { ...prior, duplicate: true }
  }
  ...
  let status = 'rejected'
  let archived = false
  const hit = findBlock(action?.blockId)
  if (action?.action === 'check' && hit) {
    try {
      const undoEntry = manualEntry('check')
      const result = applyCheckMutation(hit, action.itemId, action.done, { expectedWhen: action.expectedWhen })
      if (result.changed) keepManual(undoEntry)
      status = result.stale ? 'stale' : 'applied'
  ...
  const receipt = { id, at: new Date().toISOString(), status }
  state.pocketActions.push(receipt)
  if (state.pocketActions.length > 256) state.pocketActions.splice(0, state.pocketActions.length - 256)
  await persistEvent({
    kind: 'pocket_action',
    action: 'check',
...
// ------------------------------------------------------------- focus spans
// A focus span is one finished stretch of attention on one task. The page
// times it and December only ever hears the number, once, after it ended —
// there is nothing here to start, nothing to stop, and nothing that ticks.

/** Below the floor a click passed through a row on its way somewhere else;
    above the ceiling a window was left open overnight. Neither is a sitting,
    so neither is history. */
export const FOCUS_MIN_MS = 20 * 1000
export const FOCUS_MAX_MS = 8 * 3600 * 1000

/** How long it took, the way a person says it: 40s, 45m, 1h 20m. */
...
/** Only a thing you can actually sit down to. A tracker, a ledger, a streak
    and a note are readings and reference — you do not spend an hour on a
    number. A list holds many tasks, so it takes the item's own id. */
function focusTarget(block, itemId) {
  const wanted = String(itemId || '')
  if (block.type === 'list') {
    if (!wanted) throw new Error('name the itemId: a focus span is one item, not the whole list')
    const item = block.items.find((i) => i.id === wanted)
    if (!item) throw new Error('unknown itemId')
    return { itemId: item.id, text: item.text }
  }
  if (block.type === 'reminder') {
    if (wanted) throw new Error('a reminder is one thing and has no items')
    return { itemId: '', text: block.text }
  }
  throw new Error(`a focus span belongs to a list item or a reminder, not a ${block.type}`)
}

/** Record one completed focus span. Writes exactly one event and one line of
    page history; the task itself is untouched, because sitting with something
    is not editing it. */
export async function recordFocus({ blockId, itemId, ms } = {}) {
  const duration = Math.round(Number(ms))
  if (!Number.isFinite(duration)) throw new Error('a focus span needs a duration in milliseconds')
  if (duration < FOCUS_MIN_MS) throw new Error('that span is too short to be a sitting')
  if (duration > FOCUS_MAX_MS) throw new Error('that span is too long to be one sitting')
  const hit = findBlock(blockId)
  if (!hit) throw new Error('unknown blockId')
  const target = focusTarget(hit.block, itemId)
  const summary = `${focusPhrase(duration)} focused on: ${String(target.text || '').slice(0, 120)}`.slice(0, 200)
  state.activity = state.activity
    .concat({ at: new Date().toISOString(), space: hit.space.name, summary, kind: 'focus' })
    .slice(-30)
  await persistEvent({
    kind: 'focus',
    ...blockEvent(hit, { itemId: target.itemId || undefined, duration, summary }),
  })
  return { ok: true, space: hit.space.name, blockId: hit.block.id, itemId: target.itemId, duration, summary }
}
...
export async function editText({ spaceId, blockId, itemId, field, text }, kind = 'edit') {
  ...
  const undoEntry=manualEntry('edit')
  if (spaceId) {
...
export async function retireSpace(id) {
  ...
  const undoEntry=manualEntry('retire')
...
function archiveYearNumber(value) {
  ...
  if (!/^\d{4}$/.test(text)) throw new Error('invalid archive year')
  const year = Number(text)
  if (year < 1000 || year > 9999) throw new Error('invalid archive year')
  return year
}

function archivedState(year) {
  const y = archiveYearNumber(year)
  const file = join(DATA_DIR, 'years', `${y}.json`)
  if (!existsSync(file)) throw new Error('no such year')
  const archived = readState(file)
  if (Number(archived.yearOf) !== y) throw new Error('archive year does not match its file')
  return archived
}

/** Publish a complete archive in one filesystem operation. A hard link makes
    the fully-written temporary file visible at its final name only when that
    name does not already exist. A matching archive is an idempotent retry;
    any other existing content is immutable and stops rollover. */
async function createArchive(year, snapshot) {
  const dir = join(DATA_DIR, 'years')
  mkdirSync(dir, { recursive: true })
  const file = join(dir, `${archiveYearNumber(year)}.json`)
  if (existsSync(file)) {
    let existing
    try {
      existing = JSON.parse(readFileSync(file, 'utf8'))
    } catch {
      throw new Error(`refusing to replace unreadable ${year} archive`)
    }
    if (!isDeepStrictEqual(existing, snapshot)) throw new Error(`refusing to replace non-identical ${year} archive`)
    return file
  }

  const tmp = `${file}.${process.pid}-${Date.now()}-${uid()}.writing`
  try {
    await writeFile(tmp, JSON.stringify(snapshot, null, 2), { flag: 'wx' })
    try {
      await link(tmp, file)
    } catch (error) {
...
function carryoverGoal(goal, fromYear, toYear) {
  if (!goal || typeof goal !== 'object') return null
  const shift = (value) => {
    const match = /^(\d{4})-(\d{2})-(\d{2})$/.exec(String(value || ''))
    if (!match || Number(match[1]) !== Number(fromYear)) return value
    return localDay(anchoredDate(Number(toYear), Number(match[2]) - 1, Number(match[3])))
  }
  const carried = { ...clone(goal), from: shift(goal.from), by: shift(goal.by), setAt: shift(goal.setAt), base: 0, movedAt: null }
  delete carried.carried
  return carried
}

function carryoverTemplate(block) {
  const common = {
    type: block.type,
    title: block.title || '',
    entities: clone(Array.isArray(block.entities) ? block.entities : []),
    ...(block.watch ? { watch: clone(block.watch) } : {}),
    ...(block.goal ? { goal: clone(block.goal) } : {}),
  }
  if (block.type === 'list') {
    return { ...common, items: block.items.filter((item) => !item.done).map(({ text, src }) => ({ text, src: src || '' })) }
  }
  if (block.type === 'tracker') {
    return { ...common, current: 0, target: block.target, unit: block.unit || '', period: block.period || '' }
  }
  if (block.type === 'reminder') {
    return {
      ...common,
      ...
      at: block.at || '',
      repeat: block.repeat || '',
      repeatAnchor: block.repeatAnchor || recurrenceAnchorOf(block.when),
    }
  }
  if (block.type === 'streak') return { ...common, dates: [] }
  return null
}

function previousDay(day) {
  const date = parseLocalDay(day, 'carryover date')
  date.setDate(date.getDate() - 1)
  return localDay(date)
}

function carryoverReminderWhen(template) {
  if (!template.repeat || !template.when) return template.when || ''
  const today = localDay()
  const floor = today.startsWith(`${state.yearOf}-`) ? today : `${state.yearOf}-01-01`
  return nextWhen(template.when, template.repeat, previousDay(floor), template.repeatAnchor)
}
    ...
    for (const b of s.blocks) {
      if (b.type === 'list') {
        ...
        if (open.length) items.push({
          id: `co${n++}`, space: s.name, area: s.area || '', role: s.role || '', kind: 'list', title: b.title,
          texts: open.map((i) => i.text), template: carryoverTemplate(b),
        })
      }
      if (b.type === 'tracker' && b.current < b.target) {
        items.push({
          id: `co${n++}`, space: s.name, area: s.area || '', role: s.role || '', kind: 'tracker', title: b.title,
          target: b.target, unit: b.unit, note: `reached ${b.current} of ${b.target}`, template: carryoverTemplate(b),
        })
      }
      if (b.type === 'reminder' && !b.done) {
        items.push({
          ...
          template: carryoverTemplate(b),
        })
      }
      if (b.type === 'streak') {
        items.push({
          id: `co${n++}`, space: s.name, area: s.area || '', role: s.role || '', kind: 'streak', title: b.title,
          note: `${b.dates.length} day${b.dates.length === 1 ? '' : 's'} in ${old}`, template: carryoverTemplate(b),
        })
      }
    }
  }
...
export async function applyCarryover(ids) {
  ...
  for (const it of chosen) {
    ...
    if (!space.area && it.area) space.area = String(it.area).slice(0, 40)
    if (!space.role && (it.role === 'keep' || it.role === 'do')) space.role = it.role
    let block = null
    const template = it.template && typeof it.template === 'object' ? clone(it.template) : null
    if (template) {
      if (template.type === 'reminder') template.when = carryoverReminderWhen(template)
      const make = template.type === 'list' ? makeCarryoverBlock : makeBlock
      block = make({ ...template, goal: false, watch: false })
      if (block && template.repeatAnchor) block.repeatAnchor = template.repeatAnchor
      if (block && template.watch) block.watch = clone(template.watch)
      if (block && template.goal) block.goal = carryoverGoal(template.goal, co.fromYear, state.yearOf)
    } else {
      // Carryover created by an older December remains usable after upgrade.
      if (it.kind === 'list') block = makeCarryoverBlock({ type: 'list', title: it.title, items: it.texts })
      if (it.kind === 'tracker') block = makeBlock({ type: 'tracker', title: it.title, target: it.target, unit: it.unit, current: 0, period: 'year' })
      if (it.kind === 'reminder') {
        const when = it.repeat ? carryoverReminderWhen(it) : it.when
        block = makeBlock({ type: 'reminder', title: '', text: it.text, when, at: it.at, repeat: it.repeat })
...
export function readYear(y) {
  ...
  return { year: Number(old.yearOf), months: yearSummaryOf(old), spaces }
}
...
export function readMonth(ym) {
  ...
  const requestedYear = Number(m.slice(0, 4))
  const archive = join(DATA_DIR, 'years', `${requestedYear}.json`)
  // Legacy/live pages can contain dated history from another year. Prefer a
  // real archive when one exists; otherwise retain that established reader.
  const source = requestedYear !== Number(state.yearOf) && existsSync(archive) ? archivedState(requestedYear) : state
  ...
  const spaces = []
  const roster = [
    ...(source.spaces || []).map((space) => ({ space, retired: false })),
    ...(source.retired || []).map((space) => ({ space, retired: true })),
  ]
  for (const { space: s, retired } of roster) {
    ...
    for (const b of s.blocks) {
      ...
      if (b.type === 'reminder' && !b.done && inMonth(b.when)) {
        const isOverdue = b.when < localDay()
        lines.push({
          day: b.when,
          text: b.text,
          ...(isOverdue ? { overdue: true } : { ahead: true }),
          ...(b.at ? { at: b.at } : {}),
          ...(b.repeat ? { repeat: b.repeat } : {}),
        })
    ...
    if (ahead) bits.push(`${ahead} scheduled`)
  ...
  const overdueTotal = spaces.reduce((n, sp) => n + sp.lines.filter((l) => l.overdue).length, 0)
  const label = new Date(yy, mm - 1, 1).toLocaleString('en', { month: 'long', year: 'numeric' })
  spaces.sort((a, b) => b.lines.length - a.lines.length)
  return { month: m, label, archived: source !== state, total, ahead: aheadTotal, overdue: overdueTotal, weeks, spaces }
}
...
/** Render either the live page or an immutable archive as the same Markdown
    document. The no-argument path intentionally preserves current export. */
export function exportMarkdown(year) {
  let p
  if (year == null) {
    p = project()
  } else {
    const archived = archivedState(year)
    p = {
      year: { year: Number(archived.yearOf), months: yearSummaryOf(archived) },
      about: projectAbout(archived.about),
      spaces: [...(archived.spaces || []), ...(archived.retired || [])].map((space) => ({
        ...space,
        blocks: (space.blocks || []).map(projectBlock),
      })),
    }
  }
  const y = p.year.year
  ...
  p.year.months.forEach((month, index) => {
    if (month.events || month.highlights.length) lines.push(`- **${names[index]}**: ${month.events} moments${month.highlights.length ? ` — ${month.highlights.join('; ')}` : ''}`)
  })
  return lines.join('\n')
}
