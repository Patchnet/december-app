  ...
  return {
    ...
    spaces: [...state.spaces]
      ...
      .map((s) => {
        ...
        // the one live-count rule, stamped for the page: which tracker ticks
        // with this list. The client used to carry its own copy of
        // trackerCounting and the two could drift; now it reads the stamp.
        for (const b of s.blocks) {
          if (b.type !== 'list') continue
          const counting = trackerCounting(s, b)
          if (counting) blocks.find((x) => x.id === b.id).countedBy = counting.id
        }
        return { ...s, blocks }
      }),
...
export function liveGoals(now = new Date()) {
  ...
  for (const s of state.spaces) {
    ...
    for (const b of s.blocks) {
      if (!b.goal) continue
      const g = goalOf(b, now) // one derivation: goalOf carries gap, paceWord, quietDays
      out.push({
        ...
        pace: g.paceWord === 'met' ? 'met' : g.paceWord === 'on' ? 'on pace' : `${g.gap} ${g.paceWord}`,
        quietDays: g.quietDays,
      })
    }
  }
  return out
...
/** A tracker counts a list only when it is plainly counting THAT list: one
    ...
    every time something was bought. project() stamps the verdict onto the
    list as countedBy, so the page reads the rule instead of mirroring it. */
