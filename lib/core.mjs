...
import { uid, makeBlock, updateBlock as applyBlockPatch, projectBlock, goalMeasure, goalOf, setBlockGoal, GOAL_TYPES } from './blocks.mjs'
...
export function liveGoals(now = new Date()) {
  ...
  for (const s of state.spaces) {
    ...
    for (const b of s.blocks) {
      ...
      // "quiet" counts from the last move, or from the day the goal was set
      // if it has never moved — null read as "never moved" to the engine
      const since = g.movedAt || (g.setAt && `${g.setAt}T12:00:00`)
      const daysSince = since ? Math.max(0, Math.floor((now - new Date(since)) / 86400000)) : null
      ...
      out.push({
        ...
        pace: g.met ? 'met' : gap < 0.5 ? 'on pace' : g.diff > 0 ? `${gap} ahead` : `${gap} behind`,
        quietDays: daysSince,
      })
    }
  }
...
export async function addCapture(text, hint, source = {}) {
  ...
  const echo = state.captures.find(
    (c) => c.status === 'inbox' && c.text === capture.text && new Date(capture.at) - new Date(c.at) < 5000
  )
  if (echo) return { ...echo, duplicate: true }
...
export async function moveGoal({ space: ref, blockId, toBlockId } = {}) {
  ...
  if (to.block.goal) throw new Error(`${to.space.name} already has a goal on that block`)
  ...
  return { ok: true, space: to.space.name, blockId: to.block.id, absorbed, goal: goalOf(to.block) }
}
...
/** The next occurrence of a rhythm. Month and year steps clamp to the end
    of the target month: setMonth alone rolled "monthly on the 31st" from
    January to March 3rd, and it never found the 31st again. The result is
    the LOCAL day — toISOString read yesterday's date east of UTC+12. */
export function nextWhen(when, repeat, today = localDay()) {
  ...
  else if (repeat === 'monthly') stepMonths(1)
  else if (repeat === 'yearly') stepMonths(12)
  return localDay(d)
}
...
// A clock that leaps more than a year did not live through one. Rolling on
// its word would archive the live page under a date nobody meant; the jump
// has to hold for two looks a few minutes apart (the new-year watch provides
// the second) before it is believed. The ordinary turn — one year forward —
// rolls immediately, midnight included.
const ROLLOVER_CONFIRM_MS = 5 * 60 * 1000
let rolloverPending = null // { year, at }

/** The clock came back: it now reads BEFORE the year the page belongs to,
    the archive for the earlier year exists, and nothing has been written on
    the new page. That is the signature of a roll the clock caused, not the
    calendar — so the archived year is put back whole. A page that has been
    written on is never touched; the person decides that one. */
async function recoverSpuriousRollover(y) {
  const file = join(DATA_DIR, 'years', `${y}.json`)
  if (!existsSync(file)) return null
  const untouched =
    !state.spaces.length &&
    !state.retired?.length &&
    !state.captures.some((c) => c.status === 'filed' || c.status === 'inbox')
  if (!untouched) {
    console.error(
      `december: the clock reads ${y} but this page belongs to ${state.yearOf} and has writing on it. ` +
        `Nothing was changed. If ${state.yearOf} was a clock mistake, restore data/years/${y}.json by hand.`
    )
    return null
  }
  const restored = { ...emptyState(), ...JSON.parse(readFileSync(file, 'utf8')) }
  for (const k of Object.keys(state)) delete state[k]
  Object.assign(state, restored)
  state.about = normalizeAbout(state.about)
  await persistEvent({ kind: 'rollover_recovered', summary: `the clock came back to ${y}; the page was restored` }, y)
  console.error(`december: a wrong clock had archived ${y}; the page has been restored from data/years/${y}.json`)
  return y
}
...
export async function rolloverIfNeeded(now = new Date()) {
  ...
  if (state.yearOf > y) return recoverSpuriousRollover(y)
  if (state.yearOf === y) {
  ...
  if (y > state.yearOf + 1) {
    if (rolloverPending?.year !== y) {
      ...
      console.error(`december: the clock jumped from ${state.yearOf} to ${y}; waiting for it to hold before turning the page`)
      return null
    }
