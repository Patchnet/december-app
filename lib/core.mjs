...
export function project(settleStatus = {}) {
  return {
    ...
    surfaced: (state.surfaced || []).filter((s) => !s.until || s.until >= localDay()),
...
export function agentView() {
  ...
  return {
    ...
    surfaced: (state.surfaced || []).filter((x) => !x.until || x.until >= localDay()),
  }
}
...
export async function updateBlock(blockId, patch, expectType) {
  agentBatchGuard()
  const hit = findBlock(blockId)
  if (!hit) throw new Error('unknown blockId')
...
export async function editText({ spaceId, blockId, itemId, text }) {
