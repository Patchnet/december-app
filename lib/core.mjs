...
// The agent-batch undo snapshot used to live INSIDE the state and ride to
// disk with it — a full copy of the page written into every persist, for an
// undo that expires after ten minutes anyway. It is in-memory now, like the
// manual undo stack; files that still carry the old keys shed them here.
delete state.previous
delete state.previousAt
let previousSnap = null // { json, at } — one level, agent batches only
...
function persist() {
  ...
  // compact on purpose: pretty-printing roughly doubled every write, and
  // this file is written on every mutation. The yearly archive stays pretty.
...
/** Take one capture into the inbox without persisting: the shared intake
    for the single and batch paths. Returns the capture, or the existing
...
export async function addCapture(text, hint, source = {}) {
  const capture = intakeCapture(text, hint, source)
  if (capture.duplicate) return capture
  await persistEvent({ kind: 'capture', itemId: capture.id, summary: capture.text.slice(0, 200) })
  return capture
}

/** A brain dump lands as ONE write: every line takes the same intake as a
    single capture (echo dedupe included), the file is written once, and
    each kept line still gets its own event. Twenty-five lines used to mean
    twenty-five full rewrites of the year, in a row. */
export async function addCaptureBatch(texts, hint) {
  const kept = []
  for (const text of texts) {
    const capture = intakeCapture(text, hint)
    if (!capture.duplicate) kept.push(capture)
  }
  if (kept.length) {
    await persist()
...
/** Everything the ten-second poll would redraw for, in one cheap string:
    the last write, plus the time-driven bits that move between writes —
    urgency (a 9am reminder outranks the page AT 9am, not at the next
    mutation), the ask's 48-hour life, and surfaced lines expiring. When
    this matches what the page already has, /api/state answers "unchanged"
    instead of serializing the whole year. */
export function stateFingerprint(now = new Date()) {
  const urgency = state.spaces.map((s) => urgencyOf(s, now)).join(',')
  const askLive = state.ask && now - new Date(state.ask.at) < 48 * 3600000 ? state.ask.id : ''
  const surfacedLive = (state.surfaced || []).filter((x) => surfacedIsLive(x, localDay(now))).length
  return `${state.updatedAt}|${urgency}|${askLive}|${surfacedLive}|${undoIsFresh() ? 1 : 0}`
}
