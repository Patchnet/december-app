...
export async function addCapture(text, hint, source = {}) {
  ...
  // Filed captures are the year's record — the receipts under every row and
  // the words each month is remembered by. slice(-200) was quietly eating
  // them from the front by spring. Only the UNFILED inbox is capped now,
  // as protection against runaway automation; the filed past is kept whole
  // (and the year archives it all on rollover, so growth ends with the year).
  const inbox = state.captures.filter((c) => c.status === 'inbox')
  if (inbox.length > 200) {
    const drop = new Set(inbox.slice(0, inbox.length - 200).map((c) => c.id))
    state.captures = state.captures.filter((c) => !drop.has(c.id))
  }
...
function yearSummary() {
  ...
  // retired spaces stepped aside, but what happened in them still happened:
  // the year's record includes them (their reminders schedule nothing —
  // the scheduled sweep above walks only the live spaces)
...
export function readMonth(ym) {
  ...
  const spaces = []
  // "retire, never delete" must hold in the month view too: a retired
  // space's record stays; marked finished here so it schedules nothing
  const roster = [...state.spaces, ...(state.retired || []).map((s) => ({ ...s, finished: true }))]
