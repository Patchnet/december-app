...
async function persistEvent(event, year) {
  const provenance = state.rolloverProvenance
  if (provenance && event.kind !== 'rollover' && event.kind !== 'rollover_recovered') provenance.mutated = true
  await persist()
...
export async function addCapture(text, hint, source = {}) {
  ...
  let excess = state.captures.filter((c) => c.status === 'inbox').length - 200
  if (excess > 0) {
    state.captures = state.captures.filter((c) => {
      if (c.status !== 'inbox' || excess <= 0) return true
      excess--
      return false
    })
  }
