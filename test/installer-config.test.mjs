import { test } from 'node:test'
import assert from 'node:assert/strict'
import { existsSync, readFileSync, readdirSync } from 'node:fs'
import { dirname, join } from 'node:path'
import { fileURLToPath } from 'node:url'

const root = join(dirname(fileURLToPath(import.meta.url)), '..')
const pkg = JSON.parse(readFileSync(join(root, 'package.json'), 'utf8'))
const { build } = pkg
const { nsis } = build

test('Windows package uses the assisted NSIS wizard with a per-user default', () => {
  assert.equal(nsis.oneClick, false)
  assert.equal(nsis.perMachine, false)
  assert.equal(nsis.selectPerMachineByDefault, false)
  assert.equal(nsis.allowElevation, true)
  assert.equal(nsis.allowToChangeInstallationDirectory, true)
})

test('assisted wizard creates standard December shortcuts', () => {
  assert.equal(nsis.createDesktopShortcut, true)
  assert.equal(nsis.createStartMenuShortcut, true)
  assert.equal(nsis.shortcutName, 'December')
})

test('assisted wizard keeps standard finish and uninstall behavior', () => {
  assert.equal(nsis.runAfterFinish, true)
  assert.equal(nsis.removeDefaultUninstallWelcomePage, false)
  assert.equal(nsis.uninstallDisplayName, 'December')
})

test('installer has no custom hook for provider authentication', () => {
  assert.equal(nsis.script, undefined)
  assert.equal(nsis.include, undefined)
  assert.deepEqual(
    readdirSync(join(root, 'build')).filter((name) => /\.nsi$|\.nsh$/i.test(name)),
    []
  )
})

test('installer and uninstaller use the existing December icon', () => {
  assert.equal(nsis.installerIcon, 'build/icon.ico')
  assert.equal(nsis.uninstallerIcon, 'build/icon.ico')
  assert.equal(build.win.icon, 'build/icon.ico')
  assert.ok(existsSync(join(root, 'build', 'icon.ico')))
})

test('installer keeps the stable artifact name, package contents, and one updater dependency', () => {
  assert.equal(build.win.artifactName, '${productName}-Setup-${version}-${arch}.${ext}')
  assert.deepEqual(build.files, [
    'electron/**/*',
    'lib/**/*',
    'public/**/*',
    'server.mjs',
    'mcp-server.mjs',
    'package.json',
    'LICENSE',
    'NOTICE',
    'TRADEMARKS.md',
  ])
  assert.deepEqual(pkg.dependencies ?? {}, { 'electron-updater': '6.8.9' })
})

test('project license is Apache 2.0 and reserves December trademarks', () => {
  assert.equal(pkg.license, 'Apache-2.0')
  const license = readFileSync(join(root, 'LICENSE'), 'utf8')
  assert.match(license, /Apache License/)
  assert.match(license, /Version 2\.0/)
  const notice = readFileSync(join(root, 'NOTICE'), 'utf8')
  assert.match(notice, /trademark/i)
  assert.match(notice, /December/)
  assert.match(notice, /Patchnet AI, Inc\./)
  const marks = readFileSync(join(root, 'TRADEMARKS.md'), 'utf8')
  assert.match(marks, /trademark/i)
  assert.match(marks, /December/)
  assert.match(marks, /Patchnet AI, Inc\./)
})

test('Windows build publishes GitHub Releases only when asked', () => {
  assert.deepEqual(build.publish, {
    provider: 'github',
    owner: 'Patchnet',
    repo: 'december-app',
    releaseType: 'release',
  })
  assert.match(pkg.scripts['dist:win'], /--publish never/)
  assert.match(pkg.scripts['dist:win:publish'], /--publish always/)
  const workflow = readFileSync(join(root, '.github', 'workflows', 'windows-release.yml'), 'utf8')
  assert.match(workflow, /tags:\s*\n\s+- 'v\*'/)
  const tagCheck = workflow.indexOf('node scripts/check-release-tag.mjs')
  const buildInstaller = workflow.indexOf('npm run dist:win')
  const draftRelease = workflow.indexOf("gh release create '${{ github.ref_name }}' @assets")
  const releaseUpload = workflow.indexOf("gh release upload '${{ github.ref_name }}' $localAsset.path")
  const publishDraft = workflow.indexOf("gh release edit '${{ github.ref_name }}' --draft=false")
  const releaseSmoke = workflow.indexOf('node scripts/verify-release-assets.mjs')
  assert.ok(tagCheck >= 0, 'release workflow is missing the tag/version check')
  assert.ok(buildInstaller > tagCheck, 'release workflow must check the tag before building')
  assert.ok(draftRelease > buildInstaller, 'release workflow must create a draft with every asset')
  assert.ok(releaseUpload > draftRelease, 'release workflow must repair only missing assets on rerun')
  assert.ok(publishDraft > releaseUpload, 'release workflow must publish only after inventory verification')
  assert.ok(releaseSmoke > publishDraft, 'release workflow must smoke the published release assets last')
  assert.match(workflow, /RELEASE_TAG:\s*\$\{\{ github\.ref_name \}\}/)
  assert.match(workflow, /run: npm run dist:win\r?\n/)
  assert.doesNotMatch(workflow, /run: npm run dist:win:publish/)
  assert.match(workflow, /December-Setup-\$version-x64\.exe/)
  assert.match(workflow, /\$assets = @\(\$installer, "\$installer\.blockmap", 'release\/latest\.yml'\)/)
  assert.match(workflow, /foreach \(\$asset in \$assets\)/)
  assert.match(workflow, /gh release create[\s\S]+@assets[\s\S]+--draft --verify-tag/)
  assert.match(workflow, /Get-FileHash[^\n]+-Algorithm SHA256/)
  assert.match(workflow, /\$RemoteAsset\.state -eq 'uploaded'/)
  assert.match(workflow, /\$RemoteAsset\.size -eq \$LocalAsset\.size/)
  assert.match(workflow, /\$RemoteAsset\.digest -eq \$LocalAsset\.digest/)
  assert.match(workflow, /if \(-not \$release\.isDraft\)[\s\S]+published release has an unhealthy asset/)
  assert.match(workflow, /gh api --method DELETE \$remoteAsset\.apiUrl/)
  assert.match(workflow, /gh release upload[^\n]+\$localAsset\.path/)
  assert.match(workflow, /release has missing or unhealthy assets/)
  assert.match(workflow, /gh release edit[^\n]+--draft=false/)
  assert.doesNotMatch(workflow, /--clobber/)
  assert.match(workflow, /windows-latest/)
})
