...
test('Windows build publishes GitHub Releases only when asked', () => {
  ...
  const metadataUpload = workflow.indexOf("gh release upload '${{ github.ref_name }}' 'release/latest.yml' --clobber")
  ...
  assert.ok(metadataUpload > publish, 'release workflow must restore latest.yml after electron-builder publishes')
  assert.ok(releaseSmoke > metadataUpload, 'release workflow must smoke the published release assets last')
  assert.match(workflow, /RELEASE_TAG:\s*\$\{\{ github\.ref_name \}\}/)
  assert.match(workflow, /npm run dist:win:publish/)
  assert.match(workflow, /Test-Path -LiteralPath 'release\/latest\.yml'/)
